...
double convMeters(double meters);
double convGrams(double grams);
...
int main(void) {
	...
	for (int i = 0; i < numConv; i++) {
		scanf_s(" %lf %c", &inputVal, &inputUnit, 1);
		if (inputUnit == 'm') {
			conv = convMeters(inputVal);
			printf("%.6lf ft\n", conv);
		}

		else if (inputUnit == 'g') {
			conv = convGrams(inputVal);
			printf("%.6lf lbs\n", conv);

		}

		else {
			conv = convCelsius(inputVal);
			printf("%.6lf f\n", conv);
		}
	}
